 ...
 */
package org.springframework.batch.experimental.core.repository.support;
...
public class MongoDBJobRepositoryIntegrationTests {
	...
	void testJobExecution(@Autowired JobLauncher jobLauncher, @Autowired Job job) throws Exception {
		...
		for (Document document : jobInstancesCollection.find()) {
			System.out.println("job instance = " + document.toJson());
		}
		for (Document document : jobExecutionsCollection.find()) {
			System.out.println("job execution = " + document.toJson());
		}
		for (Document document : stepExecutionsCollection.find()) {
			System.out.println("step execution = " + document.toJson());
		}
	}
