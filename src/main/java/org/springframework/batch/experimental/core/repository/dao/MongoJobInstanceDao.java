...
public class MongoJobInstanceDao implements JobInstanceDao {
	...
	public JobInstance getJobInstance(String jobName, JobParameters jobParameters) {
		...
		org.springframework.batch.experimental.core.repository.persistence.JobInstance jobInstance = this.mongoOperations
			.findOne(query, org.springframework.batch.experimental.core.repository.persistence.JobInstance.class, COLLECTION_NAME);
	...
	public List<JobInstance> getJobInstances(String jobName, int start, int count) {
		...
		List<org.springframework.batch.experimental.core.repository.persistence.JobInstance> jobInstances = this.mongoOperations
			.find(query.with(Sort.by(sortOrder)),
					org.springframework.batch.experimental.core.repository.persistence.JobInstance.class, COLLECTION_NAME)
			.stream()
			.toList();
	...
	public JobInstance getLastJobInstance(String jobName) {
		Query query = query(where("jobName").is(jobName));
		Sort.Order sortOrder = Sort.Order.desc("jobInstanceId");
		org.springframework.batch.experimental.core.repository.persistence.JobInstance jobInstance = this.mongoOperations.findOne(
				query.with(Sort.by(sortOrder)), org.springframework.batch.experimental.core.repository.persistence.JobInstance.class,
				COLLECTION_NAME);
		return jobInstance != null ? this.jobInstanceConverter.toJobInstance(jobInstance) : null;
	}
	...
	public List<String> getJobNames() {
		return this.mongoOperations
			.findAll(org.springframework.batch.experimental.core.repository.persistence.JobInstance.class, COLLECTION_NAME)
			.stream()
			.map(org.springframework.batch.experimental.core.repository.persistence.JobInstance::getJobName)
			.toList();
	}
